...
async def test_encryption_1(dut):
    ...
    dut._log.info("reset = %d, enc_start = %d, p = %x, c = %x, enc_done = %d",
                  dut.reset.value, dut.enc_start.value, dut.p.value, dut.c.value, dut.enc_done.value)
...
def test_rc5_ca_keygen_runner():
    ...
    # RC5 RTL source files (need both modules)
    sources = [
    ...
    runner.test(hdl_toplevel="rc5_enc_16bit", test_module="test_rc5_ca_keygen")
